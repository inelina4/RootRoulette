...
class EtymologyService:
    # Apzīmējumi valodām no pirmās versijas, lai nav pilnie jāraksta
    LANGUAGE_CODE_MAP = {
        'A': 'Greek',
        'B': 'Latin', 
        'C': 'Old English',
    ...
    #Citas pieliktas klāt, lai vairāk opciju
    ALL_LANGUAGES = [
    ...
    def get_word_data_sync(self, word: str) -> Optional[WordData]:
        ...
        #Pārbauda, vai ir correct language mapping
        correct_language = self.get_correct_language(word)
        if not correct_language:
            return None
